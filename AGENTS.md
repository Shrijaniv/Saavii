...
- **The LLM never mutates durable state.** Anything in `packages/reasoning` returns structured proposals. If you find yourself persisting, notifying, or calling an external API from a reasoning path — stop, return a proposal, and let a validated service in an orchestrator execute it. The named contract for this is the **Action Protocol** (`ARCHITECTURE.md` §5; Volume II & IV): versioned proposal schemas in `contracts`, validation and execution in orchestrators — every interaction surface uses this one path, with no surface-specific shortcuts.
